...
static bool ANDROIDCAMERA_Init(SDL_CameraDriverImpl *impl)
{
    ...
    bool okay = true;
    #define LOADSYM(lib, fn) do { if (okay) { p##fn = (pfn##fn) dlsym(lib, #fn); if (!p##fn) { SDL_Log("CAMERA: symbol '%s' can't be found in %s: %s", #fn, #lib "ndk.so", dlerror()); okay = false; } } } while(false)
